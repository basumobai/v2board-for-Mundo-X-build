...
class PaytaroQR
{
    ...
    public function pay($order)
    {
        ...
        $siteUrl = !empty($order['return_url']) ? $order['return_url'] : $order['notify_url'];
        return ['type' => 1, 'data' => $this->siteBase((string) $siteUrl) . self::PAGE_PATH . '?uuid=' . rawurlencode($res['uuid'])];
    }

    public function notify($params)
    {
        $secret = isset($_SERVER['HTTP_X_APP_SECRET']) ? (string) $_SERVER['HTTP_X_APP_SECRET'] : '';
        if ($secret === '' || $this->appSecret() === '' || !hash_equals($this->appSecret(), $secret)) {
            return false;
        }
    ...
    {
        foreach (['pid', 'key', 'method_uuid'] as $key) {
            if (!isset($this->config[$key]) || trim((string) $this->config[$key]) === '') {
                abort(500, 'Paytaro 配置不完整：' . $key);
            }
        }
    }
